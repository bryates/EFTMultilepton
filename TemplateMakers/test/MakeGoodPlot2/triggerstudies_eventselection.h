...
{
    if ( !(((*preselected_leptons_intree).size() == 2 && (*tight_leptons_intree).size() == 2) ||
           ((*preselected_leptons_intree).size() == 3 && (*tight_leptons_intree).size() == 2 && (*preselected_leptons_intree)[2].obj.Pt() < (*tight_leptons_intree)[1].obj.Pt()))) {
        return false;
    }
