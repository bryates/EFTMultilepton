...
#include "EFTMultilepton/TemplateMakers/interface/HelperLeptonCore.h"
#include "EFTMultilepton/TemplateMakers/interface/GenericCollection.h"


///-------------- Kinematic Variables ------------------
#include "EFTMultilepton/TemplateMakers/interface/EveryVariable.h"
...
//   GenericCollection<BNtauCollection> tightLoosePreselectedTaus(beanHelper);
  ...
  GenericCollection<BNjetCollection> jetsClosestToLep2(beanHelper);
...
//   kinVars.push_back(&numRawJets);
  ...
  GenericCollectionSizeVariable<BNjetCollection> 
    numJetsClosestToLep1(&(jetsClosestToLep1.ptrToItems), "numJetsClosestToLep1");
  kinVars.push_back(&numJetsClosestToLep1);
  
  GenericCollectionSizeVariable<BNjetCollection>
    numJetsClosestToLep2(&(jetsClosestToLep2.ptrToItems), "numJetsClosestToLep2");
  kinVars.push_back(&numJetsClosestToLep2);
...
//     mydR("deltaR", "all_pairs", "dR_jj",
//           &(jets.ptrToItems), "jets_by_csv", 1, 4,
// 	 &(jets2.ptrToItems), "jets2_by_csv", 1, 4);
//   kinVars.push_back(&mydR);

  TwoObjectKinematic<BNjetCollection,BNleptonCollection>
    dRlep1Jet("deltaR", "closest_to", "dR_lep1_jet",
	     &(jetsClosestToLep1.ptrToItems), "jetsClosestToLep1_by_pt", 1, 99,
	     &(tightLooseLeptons.ptrToItems), "leptons_by_pt", 1, 1);
  kinVars.push_back(&dRlep1Jet);
  
  TwoObjectKinematic<BNjetCollection,BNleptonCollection>
    dRlep2Jet("deltaR", "closest_to", "dR_lep2_jet",
	      &(jetsClosestToLep2.ptrToItems), "jetsClosestToLep2_by_pt", 1, 99,
	      &(tightLooseLeptons.ptrToItems), "leptons_by_pt", 2, 2);
  kinVars.push_back(&dRlep2Jet);


//   TwoObjectKinematic<BNjetCollection,BNleptonCollection>
//     mindRlepJet2("deltaR", "min", "min_dR_lep_jet2",
// 	 &(jets2ByCSV.ptrToItems), "jets2_by_CSV", 1, 99,
// 	 &(tightLooseLeptons.ptrToItems), "leptons_by_pt", 1, 99);
//   kinVars.push_back(&mindRlepJet2);
...
//   kinVars.push_back(&myDeltaPhiMetLepLep);
  ...
  TwoObjectKinematic<BNjetCollection,BNjetCollection> myGenHiggsDijetMass("mass", "min", "genHiggs_dijet_mass",
  ...
  GenericCollectionMember<double, BNjetCollection>
    allJetPt(Reflex::Type::ByName("BNjet"), &(jetsByCSV.ptrToItems),
	     "pt", "jets_by_CSV",  KinematicVariableConstants::FLOAT_INIT, 4);
  kinVars.push_back(&allJetPt);

  GenericCollectionMember<double, BNjetCollection>
    allClosestJet1Pt(Reflex::Type::ByName("BNjet"), &(jetsClosestToLep1.ptrToItems),
             "leadCandPt", "jetsClosestToLep1_by_pt",  KinematicVariableConstants::FLOAT_INIT, 1);
  kinVars.push_back(&allClosestJet1Pt);
  
  GenericCollectionMember<double, BNjetCollection>
    allClosestJet2Pt(Reflex::Type::ByName("BNjet"), &(jetsClosestToLep2.ptrToItems),
		    "leadCandPt", "jetsClosestToLep2_by_pt",  KinematicVariableConstants::FLOAT_INIT, 1);
  kinVars.push_back(&allClosestJet2Pt);


  GenericCollectionMember<double, BNjetCollection>
    allJetEta(Reflex::Type::ByName("BNjet"), &(jetsByCSV.ptrToItems),
              "eta", "jets_by_CSV",  KinematicVariableConstants::FLOAT_INIT, 4);
  kinVars.push_back(&allJetEta);


  GenericCollectionMember<double, BNjetCollection>
    allJet2Eta(Reflex::Type::ByName("BNjet"), &(jets2ByCSV.ptrToItems),
              "eta", "jets2_by_CSV",  KinematicVariableConstants::FLOAT_INIT, 4);
  kinVars.push_back(&allJet2Eta);
  ...
  GenericCollectionMember<double, BNjetCollection>
    allJet2CSV(Reflex::Type::ByName("BNjet"),  &(jets2.ptrToItems),
	       "btagCombinedSecVertex", "jets2_by_CSV",  KinematicVariableConstants::FLOAT_INIT,4);
  kinVars.push_back(&allJet2CSV);
  ...
  for (ev.toBegin(); !ev.atEnd(); ++ev){
    ...
    //
    //////////////////////////////////////////////////////////////   
    tightLoosePreselectedMuons.initializeRawItemsSortedByPt(ev, "BNproducer","selectedPatMuons");
    tightLoosePreselectedElectrons.initializeRawItemsSortedByPt(ev, "BNproducer","selectedPatElectrons");
...
//     tightLoosePreselectedTaus.keepSelectedParticles(tauPreselectedID);
    ...
    jets2ByCSV.initializeRawItemsSortedByCSV(jets2.items);
    ...
    met.initializeRawItems(ev, "BNproducer","patMETs");
    ...
    float lep2_eta = KinematicVariableConstants::FLOAT_INIT;
    float lep2_phi = KinematicVariableConstants::FLOAT_INIT;
    ...
    if (tightLoosePreselectedLeptons.items.size() >= 2) {
      lep2_eta = tightLoosePreselectedLeptons.items.at(1)->eta;
      lep2_phi = tightLoosePreselectedLeptons.items.at(1)->phi;
    }
    float lep_jet_deltaR = 0.5;
    jetsClosestToLep1.initializeRawItems(jets.items);
    jetsClosestToLep2.initializeRawItems(jets.items);
    auto jetsClosestToLepAuto = [] (BNjet j, float x, float y, float z) { return (sqrt( pow(j.eta - x, 2)+pow(j.phi - y,2) ) < z); };
    jetsClosestToLep1.keepSelectedParticles(jetsClosestToLepAuto, lep1_eta, lep1_phi, lep_jet_deltaR);
    jetsClosestToLep2.keepSelectedParticles(jetsClosestToLepAuto, lep2_eta, lep2_phi, lep_jet_deltaR);
