...
vector<ttH::Electron> MultileptonAna::GetCollection (vecPatElectron theobjs)
{
  ...
  for (const auto & iEle: theobjs)
  {
      ...
      ele.deepcsv = iEle.userFloat("nearestJetDeepCsv");
  ...
  for(const auto & iMu: theobjs)
    {
      ...
      mu.deepcsv = iMu.userFloat("nearestJetDeepCsv");
...
vector<ttH::Jet> MultileptonAna::GetCollection (vecPatJet theobjs)
{
  ...
  for(const auto & iJet: theobjs)
  {
	...
	jet.genMotherPdgID = genMother->pdgId();
      ...
      jet.nHadFrac = iJet.neutralHadronEnergyFraction();
      jet.nEmFrac = iJet.neutralEmEnergyFraction();
      jet.nMult = iJet.neutralMultiplicity();
      jet.cMult = iJet.chargedMultiplicity();
      jet.cHadFrac = iJet.chargedHadronEnergyFraction();
